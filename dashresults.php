...
?><span class="caret"></span></a>
    ...
    if(strlen($query) >= $min_length){ 
        ...
        $raw_results = mysql_query("SELECT * FROM product
            WHERE (`title` LIKE '%".$query."%') OR (`brand` LIKE '%".$query."%')") or die(mysql_error());


        if(mysql_num_rows($raw_results) > 0){ 
            while($results = mysql_fetch_array($raw_results)){
                
                //echo "<p><h3>".$results['title']."</h3>".$results['brand']."</p>";


                echo "<p><h1> Here are the search results for: <u>" .$results['title']. " </u></h1></p>";
                echo "<p> <b> Product Name: </b>".$results['title']."</p>";
                echo "<p> <b> Product Brand: </b>".$results['brand']."</p>";
                echo "<p> <b> Product Description: </b>".$results['description']."</p>";
                echo "<p> <b> Product Price: <b>".$results['price']."</p>";
                echo "<td>";?> <img src="<?php echo $results['imageLink']; ?>" height="350px" width="450"> <?php echo "</td>";
            }
